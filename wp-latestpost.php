...
$LP['Developer'] = "false"; // The plugin will display the latest post so you can modify the template.html file.
...
define('PluginFullUrl', get_option('siteurl') . '/wp-content/plugins/' . PluginDir .'/');
 ...
 function add_notify_js(){
	 ...
	 if(isset($_GET['lpCommand']) && !IsHotlink()){
		 ob_end_clean(); // Stoping object binded plugins from getting in the way!
		 switch($_GET['lpCommand']){
		 case "getMiniPost":
		 	GetMiniPosts();
		 break;
		 case "getPostCount":
		    exit($PostCount);
		 break;
		 case "getBaseSettings":
		    if($LP['Developer'] == "true"){ $PCnt = 0; } else { $PCnt = $PostCount; }
 ...
 function GetMiniPosts(){
  ...
  foreach($myposts as $post){
   ...
   $MiniPost['Title'] = htmlentities($post->post_title);
   ...
   $MiniPost['Username'] = htmlentities($UserInfo->display_name);
